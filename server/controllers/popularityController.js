...
// 热度预测
router.get('/popularity/predict', (req, res) => {
  try {
...
// 最佳游览时间
router.get('/popularity/best-time', (req, res) => {
  try {
    const { attraction, date } = req.query;
    if (!attraction) return res.status(400).json({ code: 400, message: '请提供景点名称' });
    const targetDate = date || new Date().toISOString().split('T')[0];
    res.json({ code: 200, data: { attraction, date: targetDate, ...popularityPredictionService.getBestVisitTime(attraction, targetDate) } });
  } catch (error) {
...
// 批量行程热度预测
router.post('/popularity/trip-predictions', (req, res) => {
  try {
    ...
    const targetDate = date || new Date().toISOString().split('T')[0];
    res.json({ code: 200, data: { date: targetDate, schedule: popularityPredictionService.predictDaySchedule(attractions, targetDate, startTime || 9) } });
  } catch (error) {
...
router.post('/schedule/optimize', (req, res) => {
  try {
    ...
    if (!attractions || !Array.isArray(attractions)) return res.status(400).json({ code: 400, message: '请提供景点列表' });
    const targetDate = date || new Date().toISOString().split('T')[0];
    res.json({ code: 200, data: { date: targetDate, optimizedSchedule: popularityPredictionService.optimizeScheduleForCrowd(attractions, targetDate, preferences || {}) } });
  } catch (error) {
